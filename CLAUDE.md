...
**WP7d-1: five domain classes are facades over one class per job.** `SectionTemplate` → `SectionTemplate/{Paths, Locks, Panes, Wrappers}`, `FileManager` → `FileManager/{Root, Tree, Files, Folders}`, `SiteCss` → `SiteCss/{Root, Imports, Files}`, `DataVars` → `DataVars/{Builtins, Rows, Sources, Scope}`, `SetPreviewGenerator` keeps the browser run and delegates to `SetPreview/{Targets, FieldsetImage}`. The facade keeps only the methods other code calls (measured with grep) and a one-line docblock with `@see`; the description lives on the implementation. Moved methods are static and stateless; a `protected` helper reached from another class became `public`. A test that reached a helper by reflection (`DataVarsTest` → `walk`) now calls `DataVars\Rows::walk()`. `TailwindBake` lost 13 methods and three utility tables that nothing reached (call graph from `fromHtml`/`classes`/`peelVariants`): the compiler is the only source of CSS. 20 classes over 300 lines remain (`ServiceProvider`, `Breakpoints`, `AiChat`, `LibraryAccess`, `ComponentProps`, …).
