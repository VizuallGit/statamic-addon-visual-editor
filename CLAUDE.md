...
**WP4 is done.** What remains on the `sve` registry (34 lines) is what the standalone CP scripts read off `window.sve`; it goes with WP6. No file may define its own copy of these. If a module needs the CSRF token, the preview iframe, a `<style>` tag, a translated string or CodeMirror, it imports from `lib/`. Every lib file starts with a header saying what it owns and what it may import; a new lib file follows the same shape.

**Side scripts live in `resources/js/side/` (WP6a).** Twelve former standalone scripts are imported, verbatim IIFEs with their own run-once guards, by `side/index.js` — the first import in `addon.js`, in the order their script tags used to run. Five stay in `ServiceProvider::$scripts`: `disable-publish-stack-pin.js` and `dedupe-cp-fetch.js` patch a global (`MutationObserver`, `fetch`) and must run while the page is parsed, before Statamic's core module — a module runs too late for that; `section-meta-prefetch.js` and `lite-sections.js` read the bundle off `window.sve` (WP6b); `dock-instant-preview.js` is proven on its own by the instant-paint test. Standalone scripts cannot `import`; the side scripts can, but still carry their helper copies until WP6b moves them to `lib/`. Do not add new standalone scripts.
