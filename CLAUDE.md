...
**Side scripts live in `resources/js/side/` (WP6a/b).** Fourteen former standalone scripts are imported, verbatim IIFEs with their own run-once guards, by `side/index.js` — the first import in `addon.js`, in the order their script tags used to run. Three stay in `ServiceProvider::$scripts`: `disable-publish-stack-pin.js` and `dedupe-cp-fetch.js` patch a global (`MutationObserver`, `fetch`) and must run while the page is parsed, before Statamic's core module — a module runs too late for that; `dock-instant-preview.js` is proven on its own by the instant-paint test. `lite-sections.js` and `section-meta-prefetch.js` still read the bundle off `window.sve` (the last compat lines on the registry) and `lite-sections` wraps focus-panel's `soloSection` at runtime — WP6b-2 turns both into imports and explicit hooks. The side scripts can `import`, but still carry their helper copies until then. Do not add new standalone scripts.
