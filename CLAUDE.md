...
**Old:** `tests/browser/serve-worktree.mjs` answers requests for addon/bridge/preview/overlay-host (and their CSS) from this checkout's manifest whatever hash the installed CP asked for; every other chunk by exact name. Before that fix an old file of the requested name still on disk (`emptyOutDir: false`) was served, and two releases were "proven" against the previous build. When a test passes, check `N build files served from the working tree` is on the order of 35, not 13.
