...
${i}
${s}}`:`${s}.${e.className} {
${s}}`}function jt(e,t){return t?.length?t.map(o=>No(o,e,0)).join(`
...
`:""}${s}
`;const l=K(r,n);if(!l)return r;for(const c of[...new Set(a)].reverse()){const f=new RegExp(`(^|[^\\w-])\\.${ot(c)}\\s*\\{`,"g"),u=[];let p;for(;p=f.exec(r);){const g=p.index+p[1].length,b=r.indexOf("{",g),v=Pt(r,b);v!==-1&&u.push({from:g,to:v+1})}for(const g of u.reverse()){if(g.from>=l.from&&g.to<=l.to||!rr(r,g))continue;let b=g.from;const v=r.lastIndexOf(`
