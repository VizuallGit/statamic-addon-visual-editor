...
/**
 * The open group stands on its own, in the place of the tool that opened it.
 *
 * The tool's own icon is the first thing inside the group — "every side" is
 * the same picture as "padding" — so leaving the parent beside it showed the
 * same icon twice. Only the three that open this row are hidden: a tool whose
 * click opens a menu keeps its button, because the menu is positioned from it
 * and a hidden anchor has no position.
 */
#${d} [data-sve-css-tools] > [data-sve-css-subrow] {
  padding: 2px 6px;
  border-radius: 8px;
}
#${d} [data-sve-css-chrome][data-sve-css-sub="padding"] [data-sve-css-tool="padding"],
#${d} [data-sve-css-chrome][data-sve-css-sub="margin"] [data-sve-css-tool="margin"],
#${d} [data-sve-css-chrome][data-sve-css-sub="display"] [data-sve-css-tool="display"] {
  display: none;
}
