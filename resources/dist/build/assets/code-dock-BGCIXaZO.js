...
/**
 * One background around the tool and the children it opened.
 *
 * They are siblings in the row, so the group cannot wrap them. Instead it
 * reaches back underneath: 22px of button plus the 5px gap plus 6px of air,
 * taken off the left as margin and given back as padding, so the children
 * stay exactly where they were. The tool paints above it with its own
 * highlight off, which leaves one flat surface with the icon on it.
 *
 * 33 = 6 air + 22 button + 5 gap. The separator is the group's own, drawn
