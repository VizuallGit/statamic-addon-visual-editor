...
`)}function oi(t,e){const n=H(t,e);return n?String(t).slice(n.brace+1,n.close):""}function ps(t,e,n){const o=ni(oi(e,t.className)),s="    ".repeat(n),a=[];o.decls&&a.push(co(o.decls.replace(/;+\s*$/,";"),n+1));for(const l of o.other)a.push(co(l,n+1));for(const l of t.children)a.push(ps(l,e,n+1));const i=a.filter(Boolean).join(`
...
}`;let a=String(t||"");const i=H(a,o),l=si(s);if(i){const d=ai(a,i.from);a=a.slice(0,i.from)+ri(s,d)+a.slice(i.to)}else a=`${a.trimEnd()}${a.trim()?`
`:""}${s}
`;const c=H(a,o);if(!c)return a;for(const d of[...new Set(l)].reverse()){const f=new RegExp(`(^|[^\\w-])\\.${pe(d)}\\s*\\{`,"g"),h=[];let p;for(;p=f.exec(a);){const y=p.index+p[1].length,v=a.indexOf("{",y),E=Mt(a,v);E!==-1&&h.push({from:y,to:E+1})}for(const y of h.reverse()){if(y.from>=c.from&&y.to<=c.to||!ii(a,y))continue;let v=y.from;const E=a.lastIndexOf(`
`,v-1)+1;/^\s*$/.test(a.slice(E,v))&&(v=E);let It=y.to;a[It]===`
`&&(It+=1),a=a.slice(0,v)+a.slice(It)}}return a}function Xe(t,e){const n=String(t||"");return`${n.trimEnd()}${n.trim()?`
`:""}.${e} {
}
`}function li(t,e,n){const o=String(t||""),s=v=>v.trim().replace(/;$/,"").replace(/\s+/g," "),a=String(n||"").split(`
`).map(v=>v.trim()).filter(Boolean).map(v=>v.endsWith(";")||v.endsWith("}")?v:`${v};`);if(!a.length)return o;const i=H(o,e);if(!i)return`${o.trimEnd()}${o.trim()?`
...
`),f.style.cssText="margin:0;padding:.3em .5em;font:11px ui-monospace,SFMono-Regular,Menlo,monospace;white-space:pre-wrap",f}:void 0,apply:(f,h,p,y)=>{f.dispatch({changes:{from:p,to:y,insert:c},selection:{anchor:p+c.length}}),t.setTimeout(()=>Ln(t,c),0)}}));return{from:n.from,options:l,validFor:/^[\w-]*$/}})}}function Il(t){const e=n=>{if(!kt.length)return lt.none;const o=Ee(t),s=new vt;for(const a of Zt(n)){const i=kt.filter(l=>l.name===a.name&&l.file!==o);i.length&&s.add(a.from,a.to,lt.mark({class:"sve-cm-class-taken",attributes:{title:m(t,"class_defined_in",{file:Es(i).join(", ")})}}))}return s.finish()};return gt.define({create:n=>e(n.doc.toString()),update:(n,o)=>o.docChanged||o.effects.some(s=>s.is(As()))?e(o.state.doc.toString()):n,provide:n=>P.decorations.from(n)})}const Ol=/^\.[a-zA-Z_][\w-]*$/;function Pl(t,e,n){return String(e||"").includes(n)?ge(t).length===1:!1}function ge(t){return Et(t).filter(e=>/^@scope\b/i.test(e.prelude))}function Dl(t){const e=String(t||"");return Et(e).filter(n=>Ol.test(n.prelude)?!e.slice(n.from,n.bodyFrom-1).includes("{{"):!1).map(n=>({from:n.from,to:n.to,name:n.prelude.slice(1)}))}function zl(t,e,n){const o=String(t||"");if(!Pl(o,e,n))return o;const s=Dl(o);if(!s.length)return o;const a=ge(o)[0],i=Hl(o,a),l=s.map(p=>Wl(o.slice(p.from,p.to),o,p.from,i)).join(`
...
${xt(e,i.open)}`}),s=ze();if(s)return o(s);const a=Fd(e);return a?o(a):{at:e.length,indent:"",suffix:""}}function Fd(t){const e=String(t||"");let n=null,o=0,s=0;for(;o<e.length;){if(e[o]==="}"||e[o]===";"){o+=1,s=o;continue}if(e[o]!=="{"){o+=1;continue}const a=Mt(e,o);if(a===-1||n||(n=e.slice(s,o).trim().startsWith("@")?null:{from:s,open:o,to:a},!n))return null;o=a+1,s=o}return n}function Id(t,e){const n=e===r.cssSize?"":e;r.cssSize=n,N(t,qn,n),ot("lp:set-device",{win:t,key:n?hr(n,t):"Responsive"}),n&&Bd(t,n),r.cssValues&&ca(t),ne(t,!0),Hn(),_t(t),B(t)}function Od(t,e){r.cssState=Vn.includes(e)?e:"",N(t,un,r.cssState),S(t.document),_t(t),B(t)}function Pd(t,e){const n=t.document;S(n),e.setAttribute("data-open","");const o=n.createElement("div");o.id=x,n.body.appendChild(o),D(t,e,o),o._sveApp=z(Y,o,{kind:"choices",choices:[{value:"",label:m(t,"css_state_none"),active:!r.cssState},...Vn.map(s=>({value:s,label:He(s),active:s===r.cssState}))],onPick:s=>Od(t,s)})}function _t(t){const n=t?.document.getElementById(u)?.querySelector("[data-sve-css-head]");if(!n)return;const o=wt(),s=pt(t),a=g.css?.state.doc.toString()??"";$.tag=o?.tag||"",$.scope=Qr(o?Lt().slice(o.from,o.openTo):"")||"";const i=$.scope,l=En(t,i);$.scopeElsewhere=[...new Set(l.map(c=>String(c.file).replace(/^.*\//,"")))],$.scopeElsewhereTitle=$.scopeElsewhere.length?`${m(t,"class_defined_in",{file:$.scopeElsewhere.join(", ")})} — ${m(t,"class_defined_import")}`:"",$.onScopeImport=()=>{Ln(t,i)&&_t(t)},i&&!Ms()&&Mn(t).then(()=>_t(t)),$.canEdit=!r.lastLocked,$.onTag=c=>jr(t,c.currentTarget,o),$.state=r.cssState,$.stateLabel=r.cssState?He(r.cssState):m(t,"css_state"),$.onState=c=>Pd(t,c.currentTarget),$.onSize=c=>Id(t,c),$.sizes=[{key:"",label:m(t,"tw_size_all"),title:m(t,"css_size_all_title"),active:!r.cssSize},...s.map(c=>{const d=c.base||An(a,s,c.handle).length>0;return{key:c.handle,label:c.label,title:c.base?m(t,"css_size_base_title"):`@media ${c.media}${d?"":`  ·  ${m(t,"css_size_new")}`}`,active:r.cssSize===c.handle}})],n._sveMounted||(n._sveMounted=!0,Xt(n,_l))}Se("lp:device",t=>{const e=r.lastWin;if(!e||!fa(e.document))return;const n=bn(e).find(o=>o.device===t)?.handle||"";n!==r.cssSize&&(r.cssSize=n,N(e,qn,n),ne(e,!0),Hn(),_t(e),B(e))});function Dd(t){const e=Math.max(0,Math.round(Date.now()/1e3-t)),n=new Date(t*1e3).toLocaleTimeString(void 0,{hour:"2-digit",minute:"2-digit"});let o=n;try{const s=new Intl.RelativeTimeFormat(void 0,{numeric:"auto"});e<90?o=s.format(-e,"second"):e<5400?o=s.format(-Math.round(e/60),"minute"):e<86400?o=s.format(-Math.round(e/3600),"hour"):o=s.format(-Math.round(e/86400),"day")}catch{}return`${o} · ${n}`}function la(t,e){return t.fetch(e,{credentials:"same-origin",headers:{"X-Requested-With":"XMLHttpRequest"}})}async function zd(t,e){const n=t.document,o=mt();if(S(n),!o)return;let s=[];try{const i=await la(t,`/!/sve/section-template/history?type=${encodeURIComponent(o)}`);i.ok&&(s=(await i.json())?.entries||[])}catch{s=[]}if(!n.getElementById(u)||!n.contains(e))return;e.setAttribute("data-open","");const a=n.createElement("div");a.id=x,n.body.appendChild(a),D(t,e,a),a._sveApp=z(Y,a,{kind:"choices",choices:s.length?s.map(i=>({value:i.id,label:Dd(i.at)})):[{value:"",label:m(t,"code_dock_history_empty")}],onPick:i=>{S(n),i&&jd(t,o,i)}})}async function jd(t,e,n){if(ht())return;let o=null;try{const s=await la(t,`/!/sve/section-template/history/entry?type=${encodeURIComponent(e)}&id=${encodeURIComponent(n)}`);s.ok&&(o=await s.json())}catch{o=null}!o||ht()||(te({html:o.html??"",css:o.css??"",js:o.js??""},r.lastLocked),J(t),oe(t))}function Ut(t){const e=t?.document.getElementById(u)?.querySelector("[data-sve-code-strip]");if(!e)return;e.hidden=r.styleMode!=="tw";const n=ns(t);e.innerHTML=af,e.title=m(t,n?"tw_strip_on":"tw_strip_off"),e.setAttribute("aria-label",e.title),e.setAttribute("aria-pressed",n?"true":"false")}function Rd(t,e){const n=e.querySelector("[data-sve-code-strip]");!n||n._sveBound||(n._sveBound=!0,n.addEventListener("click",o=>{o.preventDefault(),o.stopPropagation(),Rr(t,!ns(t)),Ut(t),Hr(t)}),Ut(t))}function Hd(t,e){const n=e.querySelector("[data-sve-code-history]");!n||n._sveBound||(n._sveBound=!0,n.innerHTML=rf,n.title=m(t,"code_dock_history"),n.setAttribute("aria-label",n.title),n.addEventListener("click",o=>{if(o.preventDefault(),o.stopPropagation(),n.hasAttribute("data-open")){S(t.document);return}zd(t,n)}))}function If(){return r.styleMode}function Wd(t){return r.styleMode==="tw"?wt():null}function wt(t){const e=g.html;if(!e)return null;const n=r.htmlScopeActive&&!!r.htmlFocus,o=n?r.htmlFull:e.state.doc.toString(),a=(n?r.htmlFocus.from:0)+e.state.selection.main.from,i=es(we(o),new Set);let l=null;for(const c of i)c.from<=a&&a<c.to&&(l=c);return l}function oe(t){r.styleMode==="tw"&&Wr(t,Wd())}function Wn(t){const e=t?.document.getElementById(u),n=e?.querySelector("[data-sve-values-mode]");if(!e||!n)return;e.setAttribute("data-sve-values",r.cssValues?"on":"off");const o=t.document.createElement("span");o.textContent=m(t,"code_dock_values"),n.innerHTML=cf,n.appendChild(o),n.title=m(t,r.cssValues?"code_dock_values_off":"code_dock_values_on"),n.setAttribute("aria-label",n.title),n.setAttribute("aria-pressed",r.cssValues?"true":"false")}function ca(t){const e=g.css;if(!e||e.state.readOnly)return;const n=pt(t),o=e.state.doc.toString(),s=me(o,n,r.cssSize);if(e.focus(),s.length){const p=s[0],y=Math.min(p.bodyTo,p.bodyFrom+(o.slice(p.bodyFrom).match(/^[^\S\n]*\n?/)||[""])[0].length);e.dispatch({selection:{anchor:y},scrollIntoView:!0});return}const a=ra(t,r.cssSize);if(!a||a.base){const p=`#id-{{ id }} {
