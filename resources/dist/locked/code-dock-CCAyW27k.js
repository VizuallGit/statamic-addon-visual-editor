...
`:""}${s}
`;const c=W(a,o);if(!c)return a;for(const d of[...new Set(l)].reverse()){const f=new RegExp(`(^|[^\\w-])\\.${me(d)}\\s*\\{`,"g"),h=[];let p;for(;p=f.exec(a);){const y=p.index+p[1].length,v=a.indexOf("{",y),E=Mt(a,v);E!==-1&&h.push({from:y,to:E+1})}for(const y of h.reverse()){if(y.from>=c.from&&y.to<=c.to||!ii(a,y))continue;let v=y.from;const E=a.lastIndexOf(`
