/**
 * Paint HTML-dock class edits into Live Preview in the same frame.
 *
 * Own CP script — not addon.js. Does not touch overlay-host / preview / bridge.
 *
 * Astro/Vite updates CSS in place. The dock does the same: the design system
 * already in the Control Panel answers for one class (milliseconds), the rule
 * is appended to the iframe, then the class attribute is swapped. The PHP
 * morph still saves in the background. It is not on the paint path.
 */
(function () {
    'use strict';

    if (window.__sveDockInstantPreview === 8) {
        return;
    }
    window.__sveDockInstantPreview = 8;
    ...
    function stripAntlers(html) {
        return String(html)
            .replace(/\{\{#[\s\S]*?#\}\}/g, '')
            .replace(/\{\{[\s\S]*?\}\}/g, ANT);
    }

    function templateRoot(html) {
        var wrap = document.createElement('div');

        wrap.innerHTML = stripAntlers(html);
    ...
    function htmlCmView() {
        var el = document.querySelector(
            '#' + DOCK_ID + ' [data-sve-code-pane="html"] .cm-editor'
        );

        return el?.cmView?.view || null;
    }
