...
class StripVisualIds
{
  ...
  private function processGridItems(array $rows, array $fieldDefs): array
  {
    foreach ($rows as $idx => $row) {
      ...
      // _visual_id is intentionally NOT stripped from Grid rows. Unlike Replicator
      // sets (which regenerate via preProcess each CP load), Grid rows need a stable
      // UUID saved in the content file so it is available in the Antlers loop context
      // during Live Preview rendering (augment() only includes saved/POSTed values).
